...
export default function Header({ onProfileClick, onSettingsClick, onHelpClick, onLogoClick, onLogout, onSearchSelect, user }: HeaderProps) {
  ...
  return (
    <header className="h-16 bg-bg-surface border-b border-border-base flex items-center justify-between px-6 sticky top-0 z-50 transition-colors duration-300">
      ...
      <div className="flex items-center gap-4">
        <div ref={containerRef} className="relative w-96 mr-4">
          ...
          {showDropdown && (suggestions.length > 0 || isSearching) && (
            <div className="absolute left-0 right-0 mt-2 z-50 rounded-2xl border border-border-base bg-bg-surface shadow-xl max-h-96 overflow-y-auto">
              {isSearching && (
